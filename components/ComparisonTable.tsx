...
const Check: React.FC = () => <span className="text-green-400 font-bold">✔︎</span>;
const Cross: React.FC = () => <span className="text-red-400">✖︎</span>;
const Triangle: React.FC = () => <span className="text-yellow-400">△</span>;
            ...
            <table className="w-full min-w-[700px] border-collapse text-sm">
                <thead>
                    <tr className="border-b border-slate-700">
                        ...
                        {headers.map((header) => (
                             <th key={header} className={`p-4 text-center font-semibold ${header === "Strategy Co-Pilot" ? 'text-sky-400' : 'text-white'}`}>
                                {header}
                            </th>
                        ))}
                    </tr>
                ...
                <tbody className="divide-y divide-slate-800">
                    {rows.map((row) => (
                        <tr key={row.capability} className="hover:bg-slate-800/50">
                            <td className="p-4 text-left font-medium text-slate-300">{row.capability}</td>
